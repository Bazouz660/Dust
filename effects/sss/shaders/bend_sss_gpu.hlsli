...
struct DispatchParameters
{
	...
	// [Dust FXC port] SM5.0 forbids texture/sampler members inside a struct.
	// The original DepthTexture / OutputTexture / PointBorderSampler members
	// were removed; the access sites below now reference the globals
	// gSSSDepth / gSSSOut / gSSSSamp declared in the wrapper compute shader.
};
	...
	void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
	{
		...
		[unroll] for (i = 0; i < READ_COUNT; i++)
		{
			...
			depths.x = gSSSDepth.SampleLevel(gSSSSamp, read_xy * inParameters.InvDepthTextureSize, 0);
			depths.y = gSSSDepth.SampleLevel(gSSSSamp, (read_xy + offset_xy) * inParameters.InvDepthTextureSize, 0);
		#else
			int bias = bilinear > 0 ? 1 : -1;
			int2 offset_xy = int2(x_axis_major ? 0 : bias, x_axis_major ? bias : 0);

			depths.x = gSSSDepth.SampleLevel(gSSSSamp, read_xy * inParameters.InvDepthTextureSize, 0);
			depths.y = gSSSDepth.SampleLevel(gSSSSamp, read_xy * inParameters.InvDepthTextureSize, 0, offset_xy);
		#endif
		...
		// [Dust FXC port] The original early-out block used wave intrinsics
		// (WaveActiveAnyTrue / WaveGetLaneCount), which SM5.0 / FXC cs_5_0 does
		// not support. UseEarlyOut is treated as always-false here, so the whole
		// block is compiled out. The OTHER GroupMemoryBarrierWithGroupSync()
		// below (after the LDS writes) is SM5.0-valid and is kept.
#if 0
