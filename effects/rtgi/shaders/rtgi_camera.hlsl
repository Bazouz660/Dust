...
cbuffer CameraHistoryParams : register(b3)
{
    ...
    float previousTanHalfFov;
    float3 cameraHistoryPadding;
};
                          ...
                          out float2 previousUV, out float previousDepth)
{
    ...
    previousUV = float2(previous.x / (previous.z * ar * previousTanHalfFov) * .5 + .5,
                       .5 - previous.y / (previous.z * previousTanHalfFov) * .5);
