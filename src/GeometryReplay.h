...
namespace GeometryReplay
{
    ...
    // Caster category gates (productize). Controls which captured GBuffer categories the
    // replay cache includes: buildings (OBJECTS/DISTANT_TOWN/TRIPLANAR), characters (SKIN),
    // foliage (FOLIAGE). Takes effect next frame (the cache rebuilds every frame). TERRAIN is
    // always excluded (vertex-texture fetch; open ground has no occluders).
